...
if "%1" == "chessbox" (
    ...
    plink -batch pi@millipede "cd workspace/chessbox/controller && mkdir -p build && cd build && cmake -DCMAKE_BUILD_TYPE=Debug .. && make && echo 'Sending to Chessbox' && find . -maxdepth 1 -type f -executable -exec scp {} chesslr:workspace/chessbox/controller \;"
) else (
    echo Build only
