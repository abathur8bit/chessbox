...
if "%1" == "chessbox" (
    ...
    pscp -r -p README.md assets pi@chessbox:workspace/chessbox/controller-gui
    plink -batch pi@chessbox "kill `ps aux | grep cbgui | grep -v grep | awk -F' ' '{print $2}'`"
    plink -batch pi@millipede "cd workspace/chessbox/controller-gui && mkdir -p build && cd build && cmake -DCMAKE_BUILD_TYPE=Debug .. && make && scp cbgui chessbox:workspace/chessbox/controller-gui"
) else (
    echo Build only
